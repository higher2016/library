...
public abstract class EventDispatcher<T extends Enum<T>, E extends Event<T>, P extends Comparable<P>> {
	...
	public EventDispatcher(Class<T> eventTypeClass, Class<E> eventClass,
			Class<? extends Annotation> listenAnnotationClass) {
	 ...
	 * 事件源产生并抛出事件(事件产生后调用)
	 */
	public abstract boolean dispatchEvent(E event);
	 ...
	 * @param obj:监听者对象
	 */
	public synchronized boolean addEventListeners(Object obj) {
		if (objectSet.add(obj)) {
			return addEventListeners0(obj, obj.getClass(), false);
		} else {
			return false;
		}
	}
	 ...
	 * 适用于监听者是类的情况
	 * 
	 * @param staticClass:监听者类
	 */
	public synchronized boolean addEventListeners(Class<?> staticClass) {
		if (staticClassSet.add(staticClass)) {
			return addEventListeners0(null, staticClass, true);
		} else {
			return false;
		}
	}
	...
	private boolean addEventListeners0(Object obj, Class<? extends Object> clazz, boolean mustBeStaticOrNot) {
		...
		for (Method method : clazz.getDeclaredMethods()) {
			...
			try {
				...
				if (parameterTypes == null || parameterTypes.length != 1 || parameterTypes[0] != eventClass) {
					LoggerUtil.error(String.format("<%s> The parameter types of %s are wrong.", eventDispatcherName,
							methodDescription));
					continue;
				}
				...
				eventType_EventListeners.put(eventType,
						eventListenerList.toArray(new EventListener[eventListenerList.size()]));
				listenerNum++;
			} catch (Throwable t) {
				LoggerUtil.error(String.format("<%s> Make %s be a listener unsuccessfully.", eventDispatcherName,
						methodDescription));
			}
			if (listenerNum <= 0) {
				LoggerUtil.error(
						String.format("<%s> Listener method not found in class:%s.", eventDispatcherName, className));
				res = false;
			}
		}
		return res;
	 ...
	 *            : 事件
	 */
	protected boolean doEvent(T eventType, E event) {
		...
		for (EventListener<P> eventListener : eventListeners) {
			try {
				...
				res = false;
				LoggerUtil.error(
						String.format("<%s> invoke %s error.", eventDispatcherName, eventListener.methodDescription));
			}
		}
		return res;
	}
